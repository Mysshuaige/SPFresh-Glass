...
namespace glass {
    ...
    template <Metric metric, int DIM = 0> struct SQ8Quantizer_int8 {
        ...
        void encode(const float *from, char *to) const {
            for (int j = 0; j < d; ++j) {
                to[j] = static_cast<int8_t>(from[j]);
            }
        }
        ...
        template <int DALIGN = do_align(DIM, kAlign)> struct Computer {
            ...
            constexpr static auto dist_func = L2SqrSQ8_ext_int8;
            const SQ8Quantizer_int8 &quant;
            float *q;
            const float *mi, *dif;
            Computer(const SQ8Quantizer_int8 &quant, const float *query)
