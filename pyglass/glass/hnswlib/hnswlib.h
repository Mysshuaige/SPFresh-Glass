...
#include <intrin.h>
#include <stdexcept>
void hnswlib_cpuid(int32_t out[4], int32_t eax, int32_t ecx) {
  __cpuidex(out, eax, ecx);
}
static __int64 xgetbv(unsigned int x) { return _xgetbv(x); }
#else
#include "/home/sosp/SPFresh/AnnService/inc/Core/Common/InstructionUtils.h"
#include <stdint.h>
#include <x86intrin.h>
static void hnswlib_cpuid(int32_t cpuInfo[4], int32_t eax, int32_t ecx) {
...
[[maybe_unused]] static bool AVXCapable() {
  ...
  // CPU support
  hnswlib_cpuid(cpuInfo, 0, 0);
  int nIds = cpuInfo[0];

  bool HW_AVX = false;
  if (nIds >= 0x00000001) {
    hnswlib_cpuid(cpuInfo, 0x00000001, 0);
    HW_AVX = (cpuInfo[2] & ((int)1 << 28)) != 0;
  }

  // OS support
  hnswlib_cpuid(cpuInfo, 1, 0);
...
[[maybe_unused]] static bool AVX512Capable() {
  ...
  if (nIds >= 0x00000007) { //  AVX512 Foundation
    hnswlib_cpuid(cpuInfo, 0x00000007, 0);
