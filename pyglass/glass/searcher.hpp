...
struct SearcherBase {
  ...
  // MYS4 修改Search虚函数声明
  virtual void Search(const float *q, int k, int *preds, float* dists = nullptr) const = 0;
...
// MYS4 修改Search虚函数实现
  void Search(const float *q, int k, int *preds, float* dists = nullptr) const override {
        ...
        quant.reorder(pool, q, preds, k, dists);
    }
...
inline std::unique_ptr<SearcherBase> create_searcher(const Graph<int> &graph,
  ...
  if (m == Metric::L2) {
      //std::cout << "002初始化L2空间策略的搜索器，选定SQ8量化，空间策略是L2，传入参数graph" << std::endl;
  ...
  if (m == Metric::L2) {
    ...
    if (m == Metric::L2) {
        return std::make_unique<Searcher<SQ8FP32Quantizer<Metric::L2>>>(graph);
    }else {
        printf("Metric not suppported\n");
        return nullptr;
    }
