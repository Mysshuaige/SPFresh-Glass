...
namespace glass {
    ...
    struct SQ8FP32Quantizer {
        ...
        SQ8FP32Quantizer() = default;

        explicit SQ8FP32Quantizer(int dim)
                : d(dim), d_align(do_align(dim, kAlign)), code_size(d_align),
                  mx(d_align, -HUGE_VALF), mi(d_align, HUGE_VALF), dif(d_align), reorderer(dim) {}

        ~SQ8FP32Quantizer() { free(codes); }
...
//            }
        ...
        template <int DALIGN = do_align(DIM, kAlign)> struct Computer {
            ...
            const SQ8FP32Quantizer &quant;
            float *q;
            const float *mi, *dif;
            Computer(const SQ8FP32Quantizer &quant, const float *query)
