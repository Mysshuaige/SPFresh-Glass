...
//                    if (candidate_id == 0) continue;
    ...
    //MYS9
    //传入一个Lid和一个Rid,传出Lid是否能插入到Rid的邻居里(能插就是false)

    	float threshold = 0.6;
        inline bool checkCosineThreshold(const float* v1, const float* v2, const float* v3) {
            // 计算 vx = v1 - v2
            float vx[128];
            for (int i = 0; i < 128; ++i) {
                vx[i] = v1[i] - v2[i];
            ...
            // 计算余弦值
            float cosine = dotProduct / (normVec1 * normVec2);
            // 检查余弦相似度是否大于阈值
            return cosine > threshold;
        }
        inline void ifBeyondNeighbor(
                tableint newPointID,
                std::priority_queue<
        ...
        top_candidates) {
            ...
            float* newPointData = (float*)getDataByInternalId(newPointID);

            std::priority_queue<std::pair<dist_t, tableint>> tempNeigh;
            while (top_candidates.size() > 0) {
                //TODO：既然空间调整也要倒置，启发式也要倒置，干脆把他俩合并好了
                tempNeigh.emplace(-top_candidates.top().first,
                                  top_candidates.top().second);
                top_candidates.pop();
            }
            std::vector<tableint> possibleNeigh;

            while (tempNeigh.size()>0)
            {
                std::pair<dist_t, tableint> curent_pair = tempNeigh.top();
                tempNeigh.pop();
                float* currPoint = (float*)getDataByInternalId(curent_pair.second);

                bool ifbeyond = false;

                for (size_t idx = 0; idx < possibleNeigh.size(); idx++)
                {
                    //TODO；这里有一个优化点是：v1v3一旦计算好后只要存储，不用重复计算！vector<pair>
                    float* neighRaw = (float*)getDataByInternalId(possibleNeigh[idx]);
                    ifbeyond = checkCosineThreshold(newPointData, currPoint, neighRaw);
                    if (ifbeyond)
                        break;
                }
                if (ifbeyond)
                    continue;
                possibleNeigh.push_back(curent_pair.second);
                top_candidates.emplace(-curent_pair.first,
                                       curent_pair.second);
            }
        }
  ...
  tableint mutuallyConnectNewElement(
    ...
    ifBeyondNeighbor(cur_c, top_candidates);
